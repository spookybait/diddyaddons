...
import { snapTo, leftClick } from "../utils/PlayerUtils"

const S13PacketDestroyEntities = Java.type("net.minecraft.network.play.server.S13PacketDestroyEntities");
...
const rotationTrigger = register("packetSent", (packet, event) => {
	if (!Settings().AutoBloodCamp) return;
	if (reportRotation) {
		if (!Settings().AutoBloodCampRotations) cancel(event);
...
}).setFilteredClass(S13PacketDestroyEntities)
...
function onEntityDestroyed(mcEntity) {
	...
	if (Settings().AutoBloodCampRotations) {
		snapTo(yaw, pitch)
